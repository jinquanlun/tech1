...
**Important**: All commands must be run from the `tech-page/` directory.
...
4. **3D Shader System Details**: Advanced WebGL shader implementation in InfiniteHero
   - Custom fragment shaders with ray marching for 3D terrain generation
   - Dynamic mountain/terrain generation using fractal noise (`fbm6`)
   - Sphere-based geometry blending with `smin` functions for smooth transitions
   - Real-time glow effects around spherical geometry
   - Configurable camera positioning and terrain parameters for visual tuning
...
- Issue: Terrain appears too prominent or sphere effects are too strong
- Solution: Adjust shader parameters - reduce `fbm6` multiplier for flatter terrain, modify sphere radius/position for subtlety
- Key parameters: `swingPlane(height)` controls terrain base level, sphere position `vec3(x,y,z)` and radius affect background geometry
- Camera positioning via `rayOrigin` Y-offset controls viewing angle and perspective
...
This is a nested project structure where the main React application resides in the `tech-page/` subdirectory:

```
techpageV5/                    # Root project directory
├── CLAUDE.md                  # This documentation file
├── .gitignore                 # Git ignore rules
└── tech-page/                 # Main React application
    ├── package.json           # Dependencies and scripts
    ├── vite.config.ts         # Vite configuration
    ├── tsconfig.json          # TypeScript configuration
    ├── eslint.config.js       # ESLint configuration
    ├── tailwind.config.js     # TailwindCSS configuration
    └── src/                   # Source code (see File Organization above)
```